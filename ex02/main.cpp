...
/*   Updated: 2025/08/28 10:27:18 by ufo              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main() {
	...
	// 3) Invalid AForm constructions (too high / too low)
	try {
		title("Invalid AForm: sign grade 0");
		AForm badSign("BadSign", 0, 50);
		std::cout << badSign;
	} catch (std::exception& e) {
		std::cout << "Caught: " << e.what() << "\n";
	}
	try {
		title("Invalid AForm: exec grade 151");
		AForm badExec("BadExec", 50, 151);
		std::cout << badExec;
	} catch (std::exception& e) {
		std::cout << "Caught: " << e.what() << "\n";
	}
	...
	try {
		...
		AForm easy("Easy", 20, 30);
		std::cout << pro << easy;
		pro.signAForm(easy);      // should succeed
	...
	try {
		...
		AForm hard("Hard", 50, 40);
		std::cout << junior << hard;
		junior.signAForm(hard);   // should throw inside beSigned
		std::cout << hard;
	} catch (std::exception& e) {
